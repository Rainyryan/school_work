...
public:
    ...
    //BST* Find(BST*, BST*, int, stack<BST*>&); //stores the whole path in stack
    //BST* FindWR(BST*, int/*, stack<BST*> &*/);
...
/*
BST* BST::Find(BST* root, BST* node, int val, stack<BST*> &path){   //Records the path into stack, also records previous root if path is overlapping
    if(path.empty())                    //for when the stack is empty path.top() would cause segmentation fault
        path.push(root);
    else if(path.top() == root){        //if path is overlapping, remove it but keep a copy with node
        node = root;
        path.pop();
    }else if(node){                     //if this is the first step where paths parts ways, push the previous node in then push the current node (root) in
        path.push(node);
        node = NULL;
        path.push(root);
    }else                               //if both paths has parted ways already, just push the current node (root) in
        path.push(root);
    if(!root || root->data == val)      //recursion ending conditions: if root is NULL or had the targeted value, then return it
        return root;
    if(val > root->data)                //walk through the tree recursively until ending conditions is met
        return Find(root->right, node, val, path);
    else
        return Find(root->left, node, val, path);
}

BST* BST::FindWR(BST* root, int val, stack<BST*> &path){
    path.push(root);
    ...
    if(val > root->data){
        ...
        newroot->left = root;
        return FindWR(root->right, val);
    }
    else{
        BST* newroot = root->left;
        root->left = newroot->right;
        newroot->right = root;
        return FindWR(root->left, val);
    }
}*/
...
void Dump(stack<BST*> &A, stack<BST*> &B){  //Dumps stack A into stack B (reversing the order)
    while(!A.empty()){
        B.push(A.top());
        A.pop();
    }
...
void FindMeaty(){
    ...
    for(int i : values)                 //delete the nodes with values containing the trap index
        if(Contains(i, bi))
            tree.DeleteNT(root, i);
    /*
    stack<BST*> myS1, myS2;             //gets the shortest path from root to the sword location then to meaty's location via stacks
    tree.Find(root, NULL, sp, myS1);
    Dump(myS1, myS2);
    tree.Find(root, NULL, mp, myS2);
    Dump(myS2, myS1);
    tree.Find(root, NULL, sp, myS2);
    myS2.pop();
    Dump(myS2, myS1);
*/
    stack<BST*> myS1;
    tree.FindPath(root, sp, mp);
    cout<<endl<<"Cappo has successfully found his favorite Meaty <3"<<endl<<endl;
    cout<<"Shortest path to find Meaty :"<<endl;
    while(!myS1.empty()){               //cout the stack containing the path
        cout<<myS1.top()->data;
        myS1.pop();
        !myS1.empty() ? cout<<"->" : cout<<endl;
    }
}
